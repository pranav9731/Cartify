    ...
    return (
        <div className="space-y-6">
            <div className="card bg-base-100 shadow">
                <div className="card-body">
                    <form onSubmit={applyFilters} className="grid grid-cols-1 md:grid-cols-5 gap-3">
                        <input className="input input-bordered" placeholder="Search…" value={search} onChange={e => setSearch(e.target.value)} />
                        <input className="input input-bordered" placeholder="Min price" value={minPrice} onChange={e => setMinPrice(e.target.value)} />
                        <input className="input input-bordered" placeholder="Max price" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} />
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {products.map(p => (
                    <ProductCard key={p.id} product={p} onAdd={(id) => token ? addToCart(id) : alert('Please log in to add to cart')} />
                ))}
            </div>
        </div>
    );
